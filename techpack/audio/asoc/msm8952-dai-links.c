...
static struct snd_soc_dai_link msm8952_tasha_dai_links
    [ARRAY_SIZE(msm8952_common_fe_dai) + ARRAY_SIZE(msm8952_tasha_fe_dai) +
     ARRAY_SIZE(msm8952_tdm_fe_dai) + ARRAY_SIZE(msm8952_common_be_dai) +
     ARRAY_SIZE(msm8952_tasha_be_dai) + ARRAY_SIZE(msm8952_hdmi_dba_dai_link) +
...
int msm8952_init_wsa_dev(struct platform_device *pdev,
  ...
  ret = of_property_read_u32(pdev->dev.of_node, "qcom,wsa-max-devs",
                             &wsa_max_devs);
  if (ret) {
    dev_dbg(&pdev->dev,
  ...
  if (wsa_max_devs == 0) {
    dev_warn(&pdev->dev, "%s: Max WSA devices is 0 for this target?\n",
             __func__);
    card->num_aux_devs = 0;
    return 0;
  }
  ...
  wsa_dev_cnt =
      of_count_phandle_with_args(pdev->dev.of_node, "qcom,wsa-devs", NULL);
  ...
  ret = of_property_count_strings(pdev->dev.of_node, "qcom,wsa-aux-dev-prefix");
  if (ret != wsa_dev_cnt) {
    dev_err(&pdev->dev, "%s: expecting %d wsa prefix. Defined only %d in DT\n",
   ...
   */
  wsa881x_dev_info =
      devm_kcalloc(&pdev->dev, wsa_max_devs,
                   sizeof(struct msm895x_wsa881x_dev_info), GFP_KERNEL);
  if (!wsa881x_dev_info)
    return -ENOMEM;
  ...
  for (i = 0; i < wsa_dev_cnt; i++) {
    wsa_of_node = of_parse_phandle(pdev->dev.of_node, "qcom,wsa-devs", i);
    if (unlikely(!wsa_of_node)) {
      /* we should not be here */
      dev_err(&pdev->dev, "%s: wsa dev node is not present\n", __func__);
      ret = -EINVAL;
  ...
  if (found < wsa_max_devs) {
    dev_dbg(&pdev->dev, "%s: failed to find %d components. Found only %d\n",
  ...
  msm895x_codec_conf =
      devm_kcalloc(&pdev->dev, card->num_aux_devs,
  ...
  for (i = 0; i < card->num_aux_devs; i++) {
    dev_name_str = devm_kzalloc(&pdev->dev, DEV_NAME_STR_LEN, GFP_KERNEL);
    if (!dev_name_str) {
      ret = -ENOMEM;
    ...
    ret = of_property_read_string_index(
        pdev->dev.of_node, "qcom,wsa-aux-dev-prefix", wsa881x_dev_info[i].index,
        wsa_auxdev_name_prefix);
    if (ret) {
      dev_err(&pdev->dev, "%s: failed to read wsa aux dev prefix, ret = %d\n",
              __func__, ret);
      ret = -EINVAL;
    ...
    msm895x_aux_dev[i].codec_of_node = wsa881x_dev_info[i].of_node;
    ...
    msm895x_codec_conf[i].of_node = wsa881x_dev_info[i].of_node;
  }
...
struct snd_soc_card *populate_snd_card_dailinks(struct device *dev) {
  ...
  const char *tasha_lite[NUM_OF_TASHA_LITE_DEVICE] = {
      "msm8952-tashalite-snd-card", "msm8953-tashalite-snd-card"};
  ...
  if (strnstr(card->name, "tasha", strlen(card->name))) {
    ...
    memcpy(msm8952_tasha_dai_links + len1, msm8952_tasha_fe_dai,
           sizeof(msm8952_tasha_fe_dai));
    memcpy(msm8952_tasha_dai_links + len2, msm8952_tdm_fe_dai,
           sizeof(msm8952_tdm_fe_dai));
    memcpy(msm8952_tasha_dai_links + len3, msm8952_common_be_dai,
           sizeof(msm8952_common_be_dai));
    memcpy(msm8952_tasha_dai_links + len4, msm8952_tasha_be_dai,
